...
#             print(f"Input: sex={sex}, age={age}, height={height}, weight={weight}, hypertension={hypertension}, diabetes={diabetes}, bmi={bmi}, level={level}, fitness_goal={fitness_goal}, fitness_type={fitness_type}, waist_circumference={waist_circumference}, health_condition={health_condition}")
...
#             # Redirect with result parameters
#             query_params = '&'.join(f'{key}={value}' for key, value in result.items())
#             return redirect(url_for('recommendation') + '?' + query_params)
#         except Exception as e:
#             print(f"Error: {e}")
#             return render_template('index.html', error=str(e), unique_values=unique_values)
...
# def recommendation():
#     result = {
#         'Exercises': request.args.get('Exercises', 'N/A'),
#         'Equipment': request.args.get('Equipment', 'N/A'),
#         'Diet': request.args.get('Diet', 'N/A'),
...
# # Input fields
# sex = st.selectbox("Sex", unique_values['sex'])
# age = st.number_input("Age", min_value=0, max_value=120, value=28)
# height = st.number_input("Height (cm)", min_value=0.0, step=0.1, value=175.0)
# weight = st.number_input("Weight (kg)", min_value=0.0, step=0.1, value=72.0)
# hypertension = st.selectbox("Hypertension", unique_values['hypertension'])
# diabetes = st.selectbox("Diabetes", unique_values['diabetes'])
# bmi = st.number_input("BMI", min_value=0.0, step=0.1, value=23.5)
# level = st.selectbox("Fitness Level", unique_values['level'])
# fitness_goal = st.selectbox("Fitness Goal", unique_values['fitness_goal'])
# fitness_type = st.selectbox("Fitness Type", unique_values['fitness_type'])
# waist_circumference = st.number_input("Waist Circumference (in)", min_value=0.0, step=0.1, value=23.5)
# health_condition = st.selectbox("Health Condition", unique_values['health_condition'])
...
#         st.write(f"**Exercises:** {result['Exercises']}")
#         st.write(f"**Equipment:** {result['Equipment']}")
